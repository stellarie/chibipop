...
Expected: 20 tests pass in `layout` (6 from Task 2 + 14 here). Then `cargo test` — 77 total, zero warnings.
...
Expected: 21 tests pass in `layout`; `cargo test` is now **78** total.
...
Expected: 78 tests pass, zero warnings. This task adds no tests — its verification is Task 6's `probe`, which exercises it against real pixels.
...
Expected: 78 pass, zero warnings, golden corpus still green.
...
Expected: 80 tests pass, zero warnings.
...
Expected: zero warnings, 80 tests pass.
