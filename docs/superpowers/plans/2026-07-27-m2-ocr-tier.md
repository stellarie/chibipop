...
Expected: 9 tests pass. Then `cargo test` — 57 total, zero warnings.
...
///
/// Derives `Debug` because Task 3's `Resolved` embeds it and derives `Debug`.
#[derive(Debug)]
pub struct TextSpan {
    pub text: String,
...
Expected: 6 tests pass. Then `cargo test` — 63 total, zero warnings.
...
Expected: 21 tests pass in `layout` (6 from Task 2 + 15 here). Then `cargo test` — 78 total, zero warnings.
...
Expected: 22 tests pass in `layout`; `cargo test` is now **79** total.
...
Expected: 79 tests pass, zero warnings. This task adds no tests — its verification is Task 6's `probe`, which exercises it against real pixels.
...
Expected: 79 pass, zero warnings, golden corpus still green.
...
Expected: 81 tests pass, zero warnings.
...
Expected: zero warnings, 81 tests pass.
