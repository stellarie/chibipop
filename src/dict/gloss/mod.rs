...
pub enum StyleKey {
    ...
    /// chrome it draws around an image node, and a Dictionary's `styles.css`
    /// reaches that chrome by class. 明鏡国語辞典 leaves each gaiji node unsized
    /// and sets `width: 15em !important` on `.gloss-image-container`. Without
    /// this key, that gaiji takes the 1024 px of its SVG. `dict::sheet` folds
    /// the three keys from those rules, and only onto an image node.
    ///
    /// The value's `em` is the em of the *container*, which Yomitan sets to one
    /// base pixel unless the node declares `sizeUnits: "em"`. `ui::layout::image`
    /// resolves it. This key comes from `.gloss-image-container` or `.gloss-image`.
    ImageWidth,
    ...
    ///
    /// The link inherits the font size of the text around the picture, so
    /// this value's `em` is that text's em. 旺文社漢字典 caps a gaiji at `1em`
    /// this way. A separate key keeps the two ems apart.
    ImageLinkMaxWidth,
}
