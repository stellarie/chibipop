...
//! backends behind core's `RegionCapture` and `OcrEngine` seams
//! (ARCHITECTURE.md#workspace-and-seams).

pub mod capture;
pub mod ocr;

// Core owns the vocabulary and the layout/hit-scan logic
// (ARCHITECTURE.md#workspace-and-seams); re-exported so the modules above
// keep addressing them as `crate::text::…`, unchanged by the workspace
// split.
...
/// (ARCHITECTURE.md#ocr-engine - the Linux engine supplies 1).
