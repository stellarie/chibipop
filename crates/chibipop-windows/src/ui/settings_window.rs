...
use crate::settings::{
    SettingsForm, MAX_HEIGHT_RANGE, MAX_WIDTH_RANGE, PASSES_RANGE, SUMMARY_RANGE,
};
...
use windows::core::{w, Error, Result as WinResult, PCWSTR, PWSTR};
...
use windows::Win32::UI::Controls::{
    InitCommonControlsEx, SetScrollInfo, ICC_TAB_CLASSES, INITCOMMONCONTROLSEX,
};
...
fn tracked_dict_box(hwnd: HWND) -> Option<DictBox> {
    DICT_BOX
        .with(|c| c.get())
        .and_then(|(h, b)| (h == hwnd.0 as isize).then_some(b))
}
...
unsafe fn select_dict_row(
    ...
    unsafe {
        for (which, rows) in [
            (DictBox::Searched, searched),
            (DictBox::NotSearched, not_searched),
        ] {
            let Ok(list) = dlg_item(hwnd, dict_box_id(which)) else {
                continue;
            };
...
unsafe fn move_selected(hwnd: HWND, up: bool) {
    ...
    unsafe {
        let Some(mut searched) = list_rows(hwnd, ID_DICTS) else {
            return;
        };
        let Some(mut not_searched) = list_rows(hwnd, ID_DICTS_OFF) else {
            return;
        };
...
unsafe fn update_list_buttons(hwnd: HWND) {
    ...
    unsafe {
        ...
        let from = active_dict_box(hwnd);
        let Ok(dicts) = dlg_item(hwnd, dict_box_id(from)) else {
            return;
        };
        let cur = box_selection(hwnd, from);
...
impl SettingsWindow {
    ...
    fn rescope_dicts(&self) {
        ...
        unsafe {
            let (Some(active), Some(excluded)) = (
                list_rows(self.hwnd, ID_DICTS),
                list_rows(self.hwnd, ID_DICTS_OFF),
            ) else {
                return;
            };
            ...
            let all: Vec<String> = staged
                .dict_names
                .iter()
                .chain(staged.dict_excluded.iter())
                .cloned()
                .collect();
    ...
    unsafe fn build(&mut self, form: &SettingsForm, stale: &[String]) -> Result<i32> {
        ...
        unsafe {
            ...
            dict.push(group(
                "Dictionaries — topmost is shown first",
                y,
                dict_group_h(),
            )?);
            y += 20;
            ...
            dict.push(group_start(
                "Frequency data — how common each word is",
                y,
                freq_h,
            )?);
            y += 20;
    ...
    pub fn read(&self, template: &SettingsForm) -> SettingsForm {
        ...
        unsafe {
            ...
            let freq_names = list_rows(h, ID_FREQS).unwrap_or_else(|| template.freq_names.clone());
            let staged = self.staged.borrow();
    ...
    match (from, up) {
        ...
        (DictBox::Searched, false) => {
            if index + 1 < searched.len() {
                Some((DictBox::Searched, index + 1))
            } else if index < searched.len() && another_readable_row(searched, unreadable, index) {
...
mod tests {
    ...
    fn a_language_list_splits_and_orders_the_rows() {
        let (active, excluded) = scope_rows(&installed_two(), &["大辞林".to_string()], &[]);
    ...
    fn an_unreadable_row_stays_on_the_searched_side() {
        ...
        rows.push("broken.zip".to_string());
        let unreadable = vec!["broken.zip".to_string()];
        let (active, excluded) = scope_rows(&rows, &["大辞林".to_string()], &unreadable);
        assert_eq!(
    ...
    fn up_on_the_top_of_not_searched_crosses_to_the_bottom_of_searched() {
        ...
        let landed = dict_move(&mut searched, &mut not, &[], DictBox::NotSearched, 0, true);
    ...
    fn down_on_the_bottom_of_searched_crosses_to_the_top_of_not_searched() {
        let mut searched = names(&["A", "B"]);
        let mut not = names(&["C"]);
        let landed = dict_move(&mut searched, &mut not, &[], DictBox::Searched, 1, false);
    ...
    fn up_on_the_top_of_searched_does_nothing() {
        let mut searched = names(&["A", "B"]);
        let mut not = names(&["C"]);
        let landed = dict_move(&mut searched, &mut not, &[], DictBox::Searched, 0, true);
    ...
    fn down_on_the_bottom_of_not_searched_does_nothing() {
        ...
        let landed = dict_move(&mut searched, &mut not, &[], DictBox::NotSearched, 1, false);
        assert_eq!(None, landed);
        assert_eq!(names(&["A"]), searched);
        assert_eq!(names(&["B", "C"]), not);
    }
    ...
    fn the_last_searched_row_will_not_cross_down() {
        let mut searched = names(&["A"]);
        let mut not = names(&["B"]);
        let landed = dict_move(&mut searched, &mut not, &[], DictBox::Searched, 0, false);
        assert_eq!(None, landed);
        assert_eq!(names(&["A"]), searched);
    ...
    fn an_unreadable_row_does_not_count_toward_the_last_searched_rule() {
        ...
        let landed = dict_move(&mut searched, &mut not, &bad, DictBox::Searched, 1, false);
        assert_eq!(None, landed);
    ...
    fn up_inside_searched_reorders_without_crossing() {
        ...
        let mut not = names(&["D"]);
        let landed = dict_move(&mut searched, &mut not, &[], DictBox::Searched, 2, true);
    ...
    fn up_inside_not_searched_reorders_without_crossing() {
        ...
        let landed = dict_move(&mut searched, &mut not, &[], DictBox::NotSearched, 2, true);
    ...
    fn down_inside_not_searched_reorders_without_crossing() {
        let mut searched = names(&["A"]);
        let mut not = names(&["B", "C", "D"]);
        let landed = dict_move(&mut searched, &mut not, &[], DictBox::NotSearched, 0, false);
        assert_eq!(Some((DictBox::NotSearched, 1)), landed);
        assert_eq!(names(&["A"]), searched);
    ...
    fn only_the_searched_box_selected_acts_on_searched() {
        ...
        assert_eq!(
            DictBox::Searched,
            acting_box(true, false, Some(DictBox::NotSearched))
        );
    }
    ...
    fn both_boxes_selected_acts_on_the_last_one_touched() {
        assert_eq!(
            DictBox::NotSearched,
            acting_box(true, true, Some(DictBox::NotSearched))
        );
        assert_eq!(
            DictBox::Searched,
            acting_box(true, true, Some(DictBox::Searched))
        );
    }
    ...
    fn a_stale_tracked_box_never_beats_a_single_selection() {
        assert_eq!(
            DictBox::NotSearched,
            acting_box(false, true, Some(DictBox::Searched))
        );
    }
    ...
    fn the_dictionaries_group_did_not_outgrow_the_one_box_layout() {
        assert_eq!(
            20 + 20 + (4 * BTN_PITCH + ROW_H) + ROW_GAP + 28 + 8,
            dict_group_h()
        );
    }
