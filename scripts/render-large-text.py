...
# foreground, background, expected text after the hovered glyph, whether the
# expected text is only a prefix (the box holds part of a long line), and the hover
# height inside the glyph's ink box as a fraction from its top.
SCREENS = [
    ("shinki_100", "noto", "新規", 100, 400, 400, 0, "black", "white", None, False, 0.5),
    ("shinki_130", "noto", "新規", 130, 400, 420, 0, "black", "white", None, False, 0.5),
    ("shinki_160", "noto", "新規", 160, 400, 440, 0, "black", "white", None, False, 0.5),
    ("nihongo_130", "noto", "日本語を話す", 130, 200, 420, 2, "black", "white", None, False, 0.5),
    # The follow-up screenshot: a news line in BIZ UDPGothic, white on black.
    ("katsu_biz_100", "biz", "活発な秋雨前線や低気圧の影", 100, 120, 460, 0, "white", "black", "活発な", True, 0.5),
    ("katsu_biz_bold_125", "biz-bold", "活発な秋雨前線や低気圧の影", 125, 120, 470, 0, "white", "black", "活発な", True, 0.5),
    ("katsu_noto_100", "noto", "活発な秋雨前線や低気圧の影", 100, 120, 460, 0, "white", "black", "活発な", True, 0.5),
    # The same line, hovered near the top and near the bottom of 活. The box then
    # cuts the glyph on one side, and the engine returns garbage that fits the box.
    ("katsu_biz_110_top", "biz", "活発な秋雨前線や低気圧の影", 110, 120, 465, 0, "white", "black", "活発な", True, 0.1),
    ("katsu_biz_110_bottom", "biz", "活発な秋雨前線や低気圧の影", 110, 120, 465, 0, "white", "black", "活発な", True, 0.9),
]


def render(id_, face, text, size, x0, baseline, hovered, fg, bg, expect, prefix, hover_fy):
    ...
    return {
        ...
        "text": text,
        "chars": chars,
        "hover": {"x": hit["x"] + hit["w"] // 2, "y": hit["y"] + int(hit["h"] * hover_fy)},
