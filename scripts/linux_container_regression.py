        ...
        [
            ...
            Step(
                ...
                (
                    ...
                    "for i in $(seq 1 150); do test -S /runtime/wayland-1 && exit 0; sleep .2; done; "
                    "cat /smoke/sway.log; exit 1",
                ),
            ),
            Step(
...
def docker_exec_base(runtime: str, name: str, source_date_epoch: str = "0") -> list[str]:
    env = {
        "CARGO_HOME": "/cargo-home",
        "CARGO_TARGET_DIR": "/cargo-target",
        "XDG_RUNTIME_DIR": "/runtime",
        "WAYLAND_DISPLAY": "wayland-1",
        "SWAYSOCK": "/runtime/sway-ipc.sock",
        "WLR_BACKENDS": "headless",
        "WLR_LIBINPUT_NO_DEVICES": "1",
        "WLR_RENDERER": "pixman",
        "SOURCE_DATE_EPOCH": source_date_epoch,
    }
    command = [runtime, "exec", "--workdir", "/work/source"]
...
def validate_step(step: Step, code: int, output: str) -> tuple[str, str]:
    ...
    if step.validator == "required-prerequisite" and UNAVAILABLE_RE.search(output):
...
def workspace_copy_command(args: argparse.Namespace, name: str) -> list[str]:
    excludes = [
        ".git", ".claude", "target", "regression-artifacts",
        "linux-regression-artifacts", ".scratch",
    ]
    if args.artifact_source_exclude:
        excludes.append(args.artifact_source_exclude)
    tar_excludes = " ".join(f"--exclude={shlex.quote(value)}" for value in excludes)
    return docker_exec_base(args.runtime, name, args.source_date_epoch) + [
        "bash", "-lc",
        f"set -o pipefail; mkdir -p /work/source && tar -C /source {tar_excludes} "
        ...
        for position, step in enumerate(steps, 1):
            ...
            try:
                completed = run_process(
                    docker_exec_base(args.runtime, name, args.source_date_epoch) + list(step.command),
...
def main(argv: list[str] | None = None) -> int:
    ...
    if runtime_path is None:
        ...
        return 2
    try:
        info = run_process([args.runtime, "info", "--format", "{{json .ServerVersion}}"], 30)
    except (OSError, subprocess.TimeoutExpired) as error:
        info = subprocess.CompletedProcess([], 1, str(error))
